...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        # The min function takes the least of the two, so in this case it
        # takes whatever is left from the two values.
        x = min(self.volume - len(self.contents),
                len(additional_contents))
        for k in range (x):
            self.contents += additional_contents[k]
        ad = ''
        for k in range(x, len(additional_contents)):
            ad += additional_contents[k]
        return ad

#         using slices!
#         x = self.volume - len(self.contents)
#         self.contents += additional_contents[:x]
#         return additional_contents[x: len(additional_contents)]
    ...
    def shrink(self, new_volume):
        ...
        x = min(new_volume, len(self.contents))
        m = ''
        t = self.contents
        for k in range (x, len(self.contents)):
            m += self.contents[k]
        for k in range(x):
            content += t[k]
...
#             self.volume = new_volume
#         return len(x) + len(n)
#             The above is another code that is similar to the next one
#             but written differently.

        if (self.volume > new_volume):
            n = len(self.double())
            x = len(self.shrink(new_volume))
            self.volume = new_volume
        else:
            n = len(self.double())
            x = 0
    ...
    def reset(self):
        ...
        self.contents123 = self.contents
        if (len(self.contents) <= self.volume):
            self.volume = self.volume1
            self.contents = self.content1
        if len(self.contents) > self.volume:
            self.contents = self.content12
...
#
#         for k in range (len(other_box.contents) - x):
#             self.contents = other_box.contents[k]

        # or I can just do other_box.contents =self.append_string(other_box.contents)
    ...
    def combined_box(self, other_box):
        ...
        contentt = self.contents + other_box.contents
        volumee = self.volume + other_box.volume
        box = Box(contentt, volumee)
        return box
