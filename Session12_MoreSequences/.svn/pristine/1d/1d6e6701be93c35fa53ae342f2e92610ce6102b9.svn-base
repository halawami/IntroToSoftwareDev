...
Authors: David Mutchler, Dave Fisher, Valerie Galluzzi, Amanda Stouder,
         their colleagues and Hussein Alawami.  April 2016.
...
def test_generate_points_on_circle():
    ...
    expected = [rg.Point(130, 32),  # All numbers are approximate.
                rg.Point(110, 32), ]
    circle = rg.Circle(rg.Point(120, 32),
                       10)
...
def test_draw_points_on_circle():
    ...
    title = 'Tests 4:  2 blue on red;' + \
        '  3 yellow on unfilled.'
    window = rg.RoseWindow(440, 400, title)
    circle = rg.Circle(rg.Point(70, 60), 50)
    circle.fill_color = 'red'
    draw_points_on_circle(window, circle, 2, 'blue')
    window.continue_on_mouse_click()

    circle = rg.Circle(rg.Point(210, 210), 100)
    draw_points_on_circle(window, circle, 3, 'yellow')
    window.close_on_mouse_click()
...
def test_pizza():
    ...
    title = 'PIZZA tests 4 and 5:  2 white slices on red circle;' + \
        '  20 green1 on blue circle.'
    window = rg.RoseWindow(520, 400, title)

    circle = rg.Circle(rg.Point(125, 125), 50)
    circle.fill_color = 'red'
    pizza(window, circle, 2, 'white', 5)
    window.continue_on_mouse_click()

    circle = rg.Circle(rg.Point(350, 200), 125)
    circle.fill_color = 'white'
    pizza(window, circle, 6, 'green1', 3)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(len(pts)):
        line1 = rg.Line(pts[k], circle.center)
...
def test_polygon():
    ...
    title = 'POLYGON tests 3:  1 segment' + \
        ' with thick blue lines'
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(100, 100), 80)
    circle.outline_thickness = 3
    polygon(window, circle, 2, 'blue', 10)
    window.continue_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    line1 = rg.Line(pts[len(pts) - 1], pts[0])
    line1.attach_to(window)
    line1.thickness = thickness
    line1.color = color
...
def fancy_polygon(window, circle, number_of_lines,
    ...
    for k in range(len(pts)):
        line2 = rg.Line(pts[k], pts[(hops_to_next_point + (k)) % len(pts)])
        line2.arrow = 'last'
        line2.color = color
