...
def problem2b(rect, n, delta, win):
    ...
    rect.attach_to(win)
    corner1x = rect.get_upper_left_corner().x - delta
    corner1y = rect.get_upper_left_corner().y - delta
    corner2x = rect.get_lower_right_corner().x + delta
    corner2y = rect.get_lower_right_corner().y + delta
    for k in range(n):
