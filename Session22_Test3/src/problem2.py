...
class Gradebook(object):
    ...
    def quarter_gpa(self, quarter):
        ...
        for k in range(len(self.contents[quarter])):
            total += self.contents[quarter][k]
        gpa = total / (len(self.contents[quarter]))
        return gpa
    ...
    def total_gpa(self):
        ...
        for k in range(len(self.contents)):
            for j in range(len(self.contents[k])):
                total += self.contents[k][j]
                count += 1
    ...
    def highest_gpa(self):
        ...
        """
        index = 0
        for k in range (1, len(self.contents)):
            if  self.quarter_gpa(k) > self.quarter_gpa(index):
                index = k
        if len(self.contents) != 0:
    ...
    def how_many_to_pass(self, grade, threshold):
        ...
        count = 0
        while self.total_gpa() <= threshold:
            for k in range(len(self.contents)):
                for j in range(len(self.contents[k])):
                    if self.contents[k][j] < 4:
                        self.contents[k][j] = grade
                        count += 1
        return count
