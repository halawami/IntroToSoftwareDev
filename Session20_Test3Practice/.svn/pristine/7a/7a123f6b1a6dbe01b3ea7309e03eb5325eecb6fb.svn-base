...
def hourglass(window, n, point, radius, color):
    ...
    for k in range(n):
        for j in range(k + 1):
            circle = rg.Circle(rg.Point(x1 - (radius * k) + (diameter * j),
                                        y1 - (math.sqrt(3) * radius * k)),
                               radius)
            line = rg.Line(rg.Point(x1 - (radius * k) + (diameter * j) - (radius),
                                    y1 - (math.sqrt(3) * radius * k)),
                           rg.Point(x1 - (radius * k) + (diameter * j) + (radius),
                                    y1 - (math.sqrt(3) * radius * k)))
            ...
            window.render(0.1)
        for l in range(k + 1):
            circle = rg.Circle(rg.Point(x1 - (radius * (k)) + (diameter * (l)),
                                        y1 + (math.sqrt(3) * radius * (k))),
                               radius)
            line = rg.Line(rg.Point(x1 - (radius * k) + (diameter * l) - (radius),
                                    y1 + (math.sqrt(3) * radius * k)),
                           rg.Point(x1 - (radius * k) + (diameter * l) + (radius),
                                    y1 + (math.sqrt(3) * radius * k)))
            circle.fill_color = color
            circle.attach_to(window)
            line.attach_to(window)
...
def many_hourglasses(window, square, m, colors):
    ...
    for k in range(m):
            hourglass(window, k + 1, rg.Point(cx + (radius * (k)), cy),
                      radius, colors[k % len(colors)])
            rectangle = rg.Rectangle(rg.Point(cx - radius, cy - radius * (1 + k * math.sqrt(3))),
                                     rg.Point(cx + radius * (1 + k) + k * radius, cy + radius * (1 + k * math.sqrt(3))))
            rectangle.attach_to(window)
            window.render(0.1)
            cx += (radius * (k + 1)) + (radius * (1 + k))
