...
def problem2(center_square, window):
    ...
    center = rg.Point(center_square.get_center().x,
                      center_square.get_center().y
                      + center_square.get_height())
    center2 = rg.Point(center_square.get_center().x + center_square.get_height(),
                       center_square.get_center().y)
    for _ in range(3):
        square = rg.Square(center, center_square.get_height())
        square.outline_color = center_square.fill_color
        center.y -= center_square.get_height()
        square.attach_to(window)
    for _ in range(2):
        square = rg.Square(center2, center_square.get_height())
        square.outline_color = center_square.fill_color
        center2.x -= 2 * center_square.get_height()
        square.attach_to(window)
    window.render()
main()
