...
def test_largest_number():
    ...
    # Test 4:
    expected = 12
    answer = largest_number(([12, 1, 2], [3, 2, 1], [2, 2, 2]))
    print('Expected and actual are:', expected, answer)
...
#     return list[index]
    ...
    # second way
    indexK = get_indexK(seq_seq)
    if indexK is None:
        return None
    indexJ = 0
    for k in range(len(seq_seq)):
        for j in range(len(seq_seq[k])):
            if seq_seq[k][j] > seq_seq[indexK][indexJ]:
                indexJ = j
                indexK = k
    return seq_seq[indexK][indexJ]


def get_indexK(seq_seq):
    for k in range(len(seq_seq)):
        if len(seq_seq[k]) != 0:
            return k
    return None
...
def test_largest_negative_number():
    ...
    # Test 4:
    expected = 0
    answer = largest_negative_number(([12, 0, 2], [3, 2, 1], [2, 2, 2]))
    print('Expected and actual are:', expected, answer)
...
def largest_negative_number(seq_seq):
    ...
    # I can do the two ways here as well
    indexK = get_indexK(seq_seq)
    indexJ = 0
    if indexK is None:
        return None
